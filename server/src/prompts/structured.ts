...
export function buildStructuredTurnSystemPrompt(segment?: string | null, intentTag?: string | null): string {
  const lines = [
    ...
    '- If the user has already asked 2+ questions, follow-ups should reflect DEEPER engagement, not re-ask what was already covered. Reference specific details from THIS turn\'s response that the user hasn\'t explored yet.',
    ...
    'Follow-up contextuality (CRITICAL for natural conversation flow):',
    '- Follow-ups MUST relate to what was just discussed in THIS response, not to the general topic.',
    '- If THIS response discussed settlement savings and specific lenders, follow-ups should ask about those specific aspects, not generic score improvement.',
    '- If THIS response discussed harassment protection, follow-ups should deepen that topic (specific rights, next steps, evidence collection), not pivot to unrelated score topics.',
    '- Think: "If I were the user reading this response, what would I naturally want to know next?" -- that is the right follow-up.',
  ...
  if (segment === 'DEP') {
    lines.push(
      ...
      '- NEVER cite specific market rate ranges (e.g. "9.5% to 21%", "11% to 28%") unless this information comes from knowledge_snippets. LLM training data rates are outdated and unreliable.',
    ...
    switch (intentTag) {
      case 'INTENT_SCORE_IMPROVEMENT':
        lines.push(
          ...
          'followUps: one about reducing card usage, one about score target (750+ or 800+ depending on current score), one about exploring accelerated repayment.',
        );
        break;
      case 'INTENT_INTEREST_OPTIMISATION':
        lines.push(
          ...
          '',
          'Section 3 (both cases) - use a heading like "A Structured Way to Save More" (paragraph): naturally introduce accelerated repayment with FREED. Describe the benefit (paying off debt faster = less total interest paid) before naming the program. Do NOT invent specific savings figures.',
      ...
      case 'INTENT_GOAL_BASED_LOAN':
        lines.push(
          ...
          'formatMode: analysis',
          'MARKET RATE RULE: NEVER cite specific interest rate ranges (e.g. "9.5% to 21%") from your training data — these are outdated. Instead use qualitative framing: "With a score above 750, you\'re in the range that banks reserve for their best rates" or "A lower debt-to-income ratio strengthens your negotiating position."',
          '',
          'CONDITIONAL on profile strength:',
          '  If score >= 750 AND FOIR < 40% (strong profile):',
          '    Section 1 "Your Loan-Ready Profile" (bullet_list): highlight strong metrics with context — score vs 750 threshold, low FOIR vs 40% comfort zone, overallOnTimeRate (payment track record), closedCleanCount (clean closures demonstrate reliability), oldestAccountAgeMonths (longer history = more trust). Each bullet should explain WHY this metric helps with lenders.',
          '    Section 2 "How to Secure the Best Rate" (bullet_list): actionable steps specific to their profile — compare offers from multiple lenders before committing, negotiate using their strong profile as leverage, time applications strategically (avoid multiple enquiries), prepare documentation (income proof, employment stability). Reference enquiryCount.',
          '    redirect: /credit-score',
          '  If score < 750 OR FOIR >= 40% (needs work):',
          '    Section 1 "Where You Stand" (bullet_list): current metrics vs what lenders look for. Include card usage (overallCardUtilization), overallOnTimeRate, enquiryCount, delinquentAccountCount.',
          '    Section 2 "Steps to Strengthen Your Application" (numbered_list): specific numeric targets using per-account data. Reference repaymentPercentage for loans close to completion, specific card usage reduction targets.',
          '    redirect: /goal-tracker',
          ...
          'followUps: one about how to compare loan offers effectively, one about what lenders prioritise beyond score, one about reducing existing debt to improve their application.',
        );
        break;
    ...
    switch (intentTag) {
      case 'INTENT_SCORE_IMPROVEMENT':
        lines.push(
          ...
          'Section 3 - use a heading like "Track Your Progress" (bullet_list): Weave Goal Tracker and Credit Insights naturally into recommendations.',
          'DCP SCORE IMPACT: Since this is a DCP_Eligible user, at least one follow-up should explore how debt consolidation affects credit score (e.g. "Would consolidating my loans help or hurt my score?"). This connects score improvement to the DCP program naturally.',
      ...
      case 'INTENT_EMI_OPTIMISATION':
        lines.push(
          ...
          'formatMode: analysis',
          'EMI DATA STRATEGY: Use monthlyObligation from advisor_context as the user\'s total EMI. Do NOT stop the response to ask the user for their EMI amount. Always provide substantive advice even when per-account EMI is unavailable.',
          '',
          'If per-account estimatedEMI data is available (any account has estimatedEMI > 0):',
          ...
          '  Section 2 - use a heading like "How Consolidation Can Help" (bullet_list): explain how combining activeAccountCount loans into a single EMI with a lower interest rate works. Use monthlyObligation to illustrate potential savings. Mention the benefit of a single due date and simplified tracking.',
      ...
      case 'INTENT_EMI_STRESS':
        lines.push(
          ...
          'formatMode: analysis',
          'EMI DATA: Use monthlyObligation from advisor_context as total EMI when per-account estimatedEMI is unavailable.',
          'Section 1 - use a heading like "Where Your Money Goes Each Month" (bullet_list): list each account with lenderName, estimatedEMI (or monthlyObligation as total), outstandingAmount. Show total monthly EMI commitment and foirPercentage.',
          ...
          'Section 3 - use a heading like "A Simpler Way to Manage This" (paragraph): first explain the problem using their data: "You have X loans with ₹Y total outstanding and ₹Z monthly EMI across multiple lenders, each with different due dates." Then introduce the benefits of consolidation:',
    ...
    switch (intentTag) {
      ...
      case 'INTENT_EMI_OPTIMISATION':
        lines.push(
          ...
          'redirect: /credit-score',
          'followUps: one asking which loan to pay off first, one asking what score they need for consolidation options, one asking about negotiating a lower rate with a specific lender from their data.',
        );
        break;
      case 'INTENT_EMI_STRESS':
        lines.push(
          ...
          'Section 3 - use a heading like "Steps to Ease the Pressure" (numbered_list): specific actions to reduce payment burden WITHOUT consolidation. Focus on: paying down highest-interest accounts first, clearing small overdue amounts, improving credit score to unlock better options. Show the gap: "Your score is X, reaching 700+ would open up loan consolidation options."',

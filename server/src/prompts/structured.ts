...
export function buildStructuredTurnSystemPrompt(segment?: string | null, intentTag?: string | null): string {
  const lines = [
    ...
    '- In DRP, FREED aims to provide settlement at an estimated 45% of the enrolled debt amount -- this is a typical outcome, not a fixed guarantee.',
    ...
    switch (intentTag) {
      case 'INTENT_SCORE_IMPROVEMENT':
        lines.push(
          ...
          'formatMode: analysis',
          'Section 1 - use a heading like "Where Your Score Stands" (bullet_list): current score, gap to dynamic target (use nextScoreTarget and scoreGapToTarget from advisor_context), overallOnTimeRate, overallCardUtilization, delinquentAccountCount, enquiryCount. Present as narrative sentences.',
          'Section 2 - use a heading like "How to Improve" (bullet_list): 2-3 actionable steps using per-account data. Standard score improvement path -- do NOT suggest DRP or settlement. Focus on payment history, utilization reduction, clearing overdue via own means.',
          'Section 3 - use a heading like "Track Your Progress" (bullet_list): Weave Goal Tracker and Credit Insights naturally into recommendations.',
      ...
      case 'INTENT_DELINQUENCY_STRESS':
        lines.push(
          ...
          'formatMode: analysis',
          'Section 1 - use a heading like "What Your Accounts Show" (bullet_list): overview of overdue accounts -- list lenders, overdueAmount, maxDPD per delinquent account. Show total overdue exposure. Reference foirPercentage. Be empathetic.',
          'Section 2 - use a heading like "Steps You Can Take Right Away" (bullet_list): suggest payment prioritization (highest DPD first to prevent legal escalation), negotiating directly with lenders for restructuring, contacting lenders for hardship programs. Reference specific accounts and amounts.',
          'Section 3 - use a heading like "Building Toward a Better Position" (bullet_list): focus on score improvement path, clearing overdue amounts via own payments, using Goal Tracker to monitor progress. Explain what blocks eligibility for structured settlement programs and show the path toward qualifying.',
      ...
      case 'INTENT_GOAL_BASED_LOAN':
        lines.push(
          ...
          'formatMode: analysis',
          'Section 1 - use a heading like "Where Things Stand" (bullet_list): explain using actual data -- delinquentAccountCount, creditScore vs 700+ threshold, overdueAmount, foirPercentage. Show specific lender overdue amounts.',
          'Section 2 - use a heading like "Your Path to Getting Approved" (numbered_list): 3-4 specific steps to become loan-eligible. Focus on clearing overdue amounts via own means, improving credit score, reducing FOIR. Reference specific accounts.',
